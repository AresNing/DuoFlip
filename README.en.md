...
Current version: **0.7.0 preview**. Built with native Swift, AppKit, SwiftUI, ScreenCaptureKit, Core Image, and Metal. No third-party runtime dependencies.

Documentation is available in Chinese and English. The app interface and text in the demo are currently in Simplified Chinese.
...
4. Press **Esc** or turn off the effect to stop capture and release the desktop image immediately. DuoFlip stays in the menu bar. Choose **Quit DuoFlip** (`退出 DuoFlip`) in the popover to exit the app.

Turn off **Automatically select the built-in display** (`自动选择内置屏幕`) to use the system's screen picker for each capture session. The system sharing indicator remains visible. Automatic selection does not bypass permission checks or enable the effect when the app launches.

**Turn off the effect while meeting apps are running** (`会议软件运行时关闭效果`) is enabled by default. You can select Feishu / Lark, Tencent Meeting, Zoom, and Microsoft Teams individually. **A selected app triggers this rule whenever it is running, even if no meeting is active.** This is a conservative precaution, not precise detection of another app's screen-sharing state. Browser-based meetings are not detected. Deselect apps you keep running, or turn off the rule. DuoFlip still disables the effect if its own capture is interrupted, fails, or remains unresponsive; it does not automatically retry or resume.
...
```sh
./scripts/test.sh           # Animation, trigger-state, and meeting-app policy checks
