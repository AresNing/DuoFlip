...
合盖特效中，连续有效采样相对稳定锚点的变化小于 2°、持续满 1.5 秒后，画面用 1 秒沿原效果曲线反向恢复，结束后移除覆盖层。累计变化达到 2° 会重置停顿计时；重复时间戳不累计时间，超过 0.5 秒的采样间隔重新计时。恢复中及恢复后，角度从恢复期间的最高读数下降至少 2° 即平滑接回合盖特效；保持不动不会反复触发。重新展开、设置变更、休眠和读数失效沿用原有清理规则。停顿判断使用原始整数角度，正常动画仍使用滤波角度；极慢移动与整数读数抖动的区分需要真机调校阈值。
