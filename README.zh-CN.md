...
当前版本：**0.8.0 预览版**。原生 Swift / AppKit / SwiftUI / ScreenCaptureKit / Core Image / Metal，无第三方运行时依赖。

应用支持英文和简体中文，默认按系统的语言优先级选择。可在菜单栏浮层的「语言」中选择「跟随系统 / English / 简体中文」，立即生效；系统语言均不受支持时回退为英文。

## 动效演示

[![DuoFlip 开合动效：正常展开、缓缓合拢、重新展开](docs/assets/duoflip-demo.zh-CN.gif)](https://github.com/AresNing/DuoFlip/raw/refs/heads/main/docs/assets/duoflip-demo.zh-CN.mp4)

合拢时逐渐模糊、收拢透视并变暗，展开时恢复清晰。以 MacBook Pro 屏幕比例等比呈现，下方标注模拟开合角度。

[观看或下载高清视频 · 12 秒](https://github.com/AresNing/DuoFlip/raw/refs/heads/main/docs/assets/duoflip-demo.zh-CN.mp4)

演示使用应用现有的原生渲染代码、生成的示例桌面和模拟角度，不是真机录屏。没有采集私人桌面，也不包含原始参考视频。

## 使用

从 [GitHub Releases](https://github.com/AresNing/DuoFlip/releases) 下载 **0.8.0 预览版**：

- [DMG 安装包 · Apple Silicon](https://github.com/AresNing/DuoFlip/releases/download/v0.8.0/DuoFlip-0.8.0-arm64.dmg)
- [ZIP 压缩包](https://github.com/AresNing/DuoFlip/releases/download/v0.8.0/DuoFlip-0.8.0-arm64.zip) · [SHA256 校验文件](https://github.com/AresNing/DuoFlip/releases/download/v0.8.0/DuoFlip-0.8.0-SHA256.txt)
...
3. 屏幕展开至「触发角度 + 5°」或以上（默认 95°），再缓缓合盖即可触发。设置中可调节效果强度与触发角度；角度范围为 75°～125°、步长 1°、默认 90°，越大越早出现效果。修改后自动保存，并需先展开至新的准备角度，才允许下一次触发。
...
```sh
./scripts/test.sh           # 动画、触发状态、会议避让及语言逻辑检查
